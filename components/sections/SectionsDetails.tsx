...
import { File, Loader2, Lock, Star } from "lucide-react";
...
const SectionsDetails = ({
  ...
  const submitRating = async (rating: number) => {
    try {
      setIsLoading(true);
      await axios.post(`/api/course/${course.id}/rate`, { rating });
  ...
  return (
    <div className="px-6 py-4 flex flex-col gap-5 my-16 z-[10]">
      ...
      <div>
        ...
        {resources.map((resource) => (
          ...
          >
            <File className="h-4 w-4 mr-4 text-white bg-white" />
            <span
              style={{
