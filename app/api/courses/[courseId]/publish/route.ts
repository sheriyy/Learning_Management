...
export const POST = async (req: NextRequest, { params }: { params: { courseId: string } }) => {
    try {
        ...
        if (!course) {
            return new Response("Course not found", { status: 401 })
        }
        const isPublishedSections = course.sections.some((section) => section.isPublished)
        if (!course.title || !course.description || !course.categoryId || !course.subCategoryId || !course.levelId || !course.imageUrl || !course.price || !isPublishedSections) {
